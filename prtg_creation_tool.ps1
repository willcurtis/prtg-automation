...
.SYNOPSIS
 ...
 Create (or reuse) groups (including nested paths), add devices, and ensure a Ping sensor per device in PRTG.

.CSV COLUMNS (v4)
 DeviceName, IP, GroupId, GroupName, ParentGroupId, GroupPath
 - Use GroupId when known (device will be added to this group).
 - Otherwise:
     • Provide ParentGroupId (a probe or group) PLUS one of:
         - GroupName (single-level subgroup), OR
         - GroupPath (nested path like "Region/City/Switches"; will create/reuse each level).
 - If both GroupName and GroupPath are provided, GroupPath takes precedence.

.PARAMETERS
 -BaseUrl         e.g. https://prtg.example.com:1616 or https://prtg.example.com:1616/api/v2
 -ApiKey          PRTG API key with write rights
 -CsvPath         Path to CSV
 -ClassicBaseUrl  e.g. https://prtg.example.com:8443/   (Core UI/API)
 -ClassicUser     (optional) classic API username (for username+passhash auth)
 -ClassicPasshash (optional) classic API passhash
...
# Logs directory: <script folder>\logs\prtg_creation_tool_v4_YYYYMMDD_HHMMSS.log
...
$global:PrtgLogFile = Join-Path $LogDir ("prtg_creation_tool_v4_{0}.log" -f $ts)
...
try {
  ...
  try {
    ...
    $global:PrtgLogFile = Join-Path $tmp ("prtg_creation_tool_v4_{0}.log" -f $ts)
    Start-Transcript -Path $global:PrtgLogFile -Append -ErrorAction Stop | Out-Null
    $global:TranscriptStarted = $true
  ...
  try {
    ...
    elseif (-not [string]::IsNullOrWhiteSpace($groupPath)) {
      if ([string]::IsNullOrWhiteSpace($parentGroupId)) {
        throw "ParentGroupId is required when using GroupPath."
      }
